...
function ProductDetail() {
  ...
  // whenever data changes, find the product with matching id
  useEffect(() => {
    if (data) {
      // find product whose id matches route id
      const detailOfProduct = data.find((e) => e.id == Number(id))
      // store found product in state
  ...
  // create array of all valid product ids
  const ValidId = data.map((e) => e.id)

  // if id in params does not exist in product list
  ...
  // scroll page to top whenever product detail page opens
  window.scrollTo({
    top: 0,
    behavior: 'smooth'
  })
    ...
    <div className="m-6">
      ...
      <div className="bg-white p-6 rounded-4xl flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-1/2 flex flex-col items-center ">
          ...
          {/* loop through all additional images */}
        ...
        {/* title */}
          ...
          {/* Stock status */}
          <p className={`font-semibold ${productDetail?.stock > 0 ? "text-green-600" : "text-red-500"
              }`} >
          ...
          {/* shipping, return and warranty info */}
          ...
          <div className="mt-8 bg-white rounded-3xl shadow-[0px_0px_12px_gray_inset] p-6 w-full">
            ...
            {/* loop through all customer reviews */}
            {productDetail?.reviews?.map((e, index) => (
              <div key={index} className="border-b py-4">

                {/* Name and rating of customer */}
